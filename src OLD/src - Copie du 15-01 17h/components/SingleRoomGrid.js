...
function SingleRoomGrid({ selectedRoom, editingReservation, onBack, onSuccess }) {
  ...
  // ✅ Pré-remplir la date de fin de récurrence avec la date de début de réservation
  useEffect(() => {
    if (formData.recurrence && !formData.recurrenceJusquau && selections.length > 0) {
      const dateDebut = googleSheetsService.formatDate(selections[0].date);
      setFormData(prev => ({ ...prev, recurrenceJusquau: dateDebut }));
    }
  }, [formData.recurrence, selections.length]);
