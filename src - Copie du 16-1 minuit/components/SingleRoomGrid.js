...
function SingleRoomGrid({ selectedRoom, editingReservation, onBack, onSuccess }) {
  ...
  // ✅ NOUVEL ÉTAT : Pour savoir si on a déjà nettoyé la sélection initiale en mode édition
  ...
  const handleMouseDown = (dayIndex, hour, date, event) => {
    ...
    // ✅ CORRECTION CRITIQUE : On efface la sélection UNIQUEMENT au premier clic de modification
    // Si on a déjà effacé (hasClearedEditingSelection === true), on n'efface plus pour permettre l'ajout multiple (responsive)
    if (editingReservation && !hasClearedEditingSelection) {
      setSelections([]);
      setHasClearedEditingSelection(true); // On marque que le nettoyage est fait
    }
  ...
  useEffect(() => {
    if (editingReservation) {
      ...
      setSelections(newSelections);
      setShowForm(true);
      setHasClearedEditingSelection(false); // ✅ RESET QUAND ON COMMENCE UNE NOUVELLE ÉDITION
    }
  }, [editingReservation]);
  ...
  return (
    <>
      {successModal.show && createPortal(
        <div className="success-modal-overlay" onClick={() => setSuccessModal({ ...successModal, show: false })} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingBottom: '70px' }}>
          <div className="success-modal" onClick={e => e.stopPropagation()}>
            <div className="success-modal-header"><h2>{successModal.reservations.length > 1 ? "Réservations confirmées !" : "Réservation confirmée !"}</h2></div>
            <div className="success-modal-body">
              ...
              <div className="ical-download-section"><button className="download-ical-button" onClick={() => icalService.generateAndDownload(successModal.reservations)}>📥 Télécharger .ics</button></div>
            </div>
            <div className="success-modal-footer"><button className="close-modal-button" onClick={() => { setSuccessModal({ ...successModal, show: false }); onSuccess && onSuccess(); }}>Fermer</button></div>
          </div>
        </div>, document.body
      )}
      ...
      <div className="single-room-container">
        ...
        {hoveredReservation && (
          <div className={`reservation-popup-card ${isFading ? 'fading-out' : ''}`} style={{ position: 'fixed', left: popupPosition.x, top: popupPosition.y, transform: popupPosition.transform, zIndex: 10001 }} onClick={() => setHoveredReservation(null)}>
            ...
            <div className="popup-card-body">
              <div className="popup-info-line"><span className="popup-info-icon">🏢</span> <strong>Service :</strong> {hoveredReservation.service}</div>
              <div className="popup-info-line"><span className="popup-info-icon">📧</span> <strong>Email :</strong> {hoveredReservation.email}</div>
              <div className="popup-info-line"><span className="popup-info-icon">📝</span> <strong>Objet :</strong> {hoveredReservation.objet}</div>
              ...
              {(hoveredReservation.salle.includes('Conseil') || hoveredReservation.salle.includes('Mariages')) && (
                <>
                  <div className="popup-info-line"><span className="popup-info-icon">🪑</span> <strong>Agencement :</strong> {hoveredReservation.agencement || 'N/A'}</div>
                  <div className="popup-info-line"><span className="popup-info-icon">👥</span> <strong>Nb Personnes :</strong> {hoveredReservation.nbPersonnes || 'N/A'}</div>
                </>
              )}
            </div>
          </div>
        ...
        {isSubmitting && <div className="modal-overlay"><div className="modal-content"><h3>Enregistrement...</h3><div style={{width:'100%',background:'#eee',height:'10px',borderRadius:'5px'}}><div style={{width:`${(submissionProgress.current/submissionProgress.total)*100}%`,background:'#4caf50',height:'100%'}}></div></div></div></div>}
