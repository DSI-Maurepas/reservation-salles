...
function ReservationGrid({ selectedDate, editReservationId, onBack, onSuccess }) {
  ...
  // REF POUR LE SCROLL AUTOMATIQUE VERS LA FICHE
  ...
  // NOUVELLE FONCTION POUR GERER LE CLIC SUR UNE SALLE ET SCROLLER
  const handleRoomClick = (salle) => {
    setHoveredSalle(salle);
    // Petit délai pour laisser le rendu se faire, puis scroll
    setTimeout(() => {
        if (detailsRef.current) {
            // Calcule la position absolue de l'élément + scroll actuel - un peu de marge
            const y = detailsRef.current.getBoundingClientRect().top + window.pageYOffset - 20;
    ...
    SALLES.forEach((salle, idx) => {
      ...
      grid.push(
        <div 
            ...
            style={{gridColumn: idx+2, cursor: 'pointer'}} // Curseur main
            onMouseEnter={() => setHoveredSalle(salle)} 
            onMouseLeave={() => setHoveredSalle(null)}
            onClick={() => handleRoomClick(salle)} // CLIC MOBILE
        >
  ...
  return (
    ...
    <div className="reservation-grid-container">
      ...
      <div className="reservation-content" onMouseUp={handleMouseUp}>
        ...
        {/* AJOUT DE LA REF SUR LA COLONNE DÉTAILS */}
