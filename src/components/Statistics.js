...
function Statistics({ reservations }) {
  const stats = useMemo(() => {
    ...
    const parService = {}; reservations.forEach(res => { parService[res.service] = (parService[res.service] || 0) + 1; });
    ...
    let totalMinutes = 0, countRes = 0; reservations.forEach(res => { if (!res.heureDebut || !res.heureFin) return; const [hS, mS] = res.heureDebut.split(':').map(Number); const [hE, mE] = res.heureFin.split(':').map(Number); const dur = (hE*60+mE) - (hS*60+mS); if(dur>0){ totalMinutes+=dur; countRes++; }});
    let dureeMoyenne = "0h00m"; if(countRes>0) { const avg = totalMinutes/countRes; dureeMoyenne = `${Math.floor(avg/60)}h${Math.round(avg%60).toString().padStart(2,'0')}m`; }
  ...
  return (
    <div className="statistics-container" onClick={() => setHoveredSlice(null)}>
      ...
      <div className="stats-summary">
        <div className="summary-card"><div className="summary-icon">📅</div><div className="summary-content"><div className="summary-value">{stats.total}</div><div className="summary-label">Total</div></div></div>
        <div className="summary-card"><div className="summary-icon">⏱️</div><div className="summary-content"><div className="summary-value">{stats.dureeMoyenne}</div><div className="summary-label">Moyenne</div></div></div>
        ...
        <div className="summary-card"><div className="summary-icon">🏢</div><div className="summary-content"><div className="summary-value">{Object.keys(stats.parSalle).length}</div><div className="summary-label">Salles</div></div></div>
      </div>
      <div className="charts-grid">
        ...
        <div className="chart-card"><h3>📊 Taux d'occupation</h3><div className="occupation-bars">{Object.entries(stats.tauxOccupation).sort((a,b)=>b[1]-a[1]).map(([s,t],i)=>(<div key={i} className="occupation-item"><div className="occupation-label">{s}</div><div className="occupation-bar-container"><div className="occupation-bar" style={{width:`${t}%`,backgroundColor:t>80?'#f44336':t>50?'#ff9800':'#4caf50'}}></div></div><div className="occupation-value">{t}%</div></div>))}</div></div>
        ...
        <div className="chart-card"><h3>👥 Top 10</h3><div className="top-users-list">{stats.topUtilisateurs.map(([n,c],i)=>(<div key={i} className="top-user-item"><span className="user-rank">{i+1}</span><span className="user-name">{n}</span><span className="user-count">{c}</span></div>))}</div></div>
      </div>
    </div>
  );
}
