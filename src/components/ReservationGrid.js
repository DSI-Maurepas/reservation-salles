...
function ReservationGrid({ selectedDate, editReservationId, onBack, onSuccess }) {
  ...
  const isUserAdmin = (email) => ADMINISTRATEURS.includes(email?.toLowerCase());
  ...
  const loadReservations = useCallback(async () => {
    setLoading(true);
    try {
      if (isDateBlocked(currentDate)) setBlockedDayModal(true);
      const all = await googleSheetsService.getAllReservations();
      const dStr = googleSheetsService.formatDate(currentDate);
      let dayRes = all.filter(res => {
        if (res.statut === 'cancelled') return false;
        const isSameDay = res.dateDebut === dStr;
        const isMultiDay = (res.dateDebut <= dStr && res.dateFin >= dStr);
        return isSameDay || isMultiDay;
      });
      if (editingReservation?.id) dayRes = dayRes.filter(r => r.id !== editingReservation.id);
      setReservations(dayRes);
    } catch (e) { console.error(e); } finally { setLoading(false); }
  }, [currentDate, editingReservation, isDateBlocked]);
  ...
  useEffect(() => {
    const loadEdit = async () => {
      if (editReservationId) {
        const all = await googleSheetsService.getAllReservations();
        const res = all.find(r => r.id === editReservationId);
        if (res) {
          setEditingReservation(res); setIsEditMode(true); setSelections([]); setCurrentSelection(null);
          setFormData({
            nom: res.nom, prenom: res.prenom, email: res.email, telephone: res.telephone,
            service: res.service, objet: res.objet, description: res.description,
            recurrence: false, recurrenceJusquau: '', recurrenceType: 'weekly', 
            agencement: res.agencement || '', nbPersonnes: res.nbPersonnes || ''
          });
        }
      }
    };
    if (editReservationId) loadEdit();
  }, [editReservationId]);

  // --- LOGIQUE GRILLE ---
  const isSlotReserved = (salle, slotStart) => {
    const slotEnd = slotStart + 0.5;
    return reservations.some(res => {
      if (normalizeRoomName(res.salle) !== normalizeRoomName(salle)) return false;
  ...
  const getReservation = (salle, slotStart) => {
    const slotEnd = slotStart + 0.5;
  ...
  const isSlotSelected = (salle, hour) => {
    return selections.some(sel => sel.salle === salle && hour >= sel.startHour && hour < sel.endHour);
  };

  const handleMouseDown = (salle, hour) => {
    if (isDateBlocked(currentDate)) { setBlockedDayModal(true); return; }
    if (isDateInPast(currentDate)) { alert('Date passée.'); return; }
    if (isSlotReserved(salle, hour)) return;
    if (isAdminOnlyRoom(salle) && !isAdminUnlocked) {
      setAdminPasswordModal({ show: true, salle, slot: {salle, hour}, password: '' });
      return;
    }
    setIsDragging(true); 
  ...
  const handleMouseEnter = (salle, hour) => {
    ...
    const currentEnd = hour + 0.5;
    let start = currentSelection.startHour;
    let end = currentSelection.endHour;
    
    if (hour < start) start = hour;
    if (currentEnd > end) end = currentEnd;

    for (let h = start; h < end; h += 0.5) {
      if (isSlotReserved(salle, h)) return;
    }
    
    setCurrentSelection({ ...currentSelection, startHour: start, endHour: end });
  };
    ...
    if (isDragging && currentSelection) {
      setSelections(prev => {
        const exists = prev.some(s => 
          s.salle === currentSelection.salle && 
          Math.abs(s.startHour - currentSelection.startHour) < 0.001 &&
          Math.abs(s.endHour - currentSelection.endHour) < 0.001
        );
        const overlap = prev.some(s => 
          s.salle === currentSelection.salle && 
          !exists && 
          ((currentSelection.startHour < s.endHour) && (currentSelection.endHour > s.startHour))
        );

        if (overlap) { alert("Chevauchement détecté."); return prev; }
  ...
  const removeMergedSelection = (mergedSlot) => {
    setSelections(currentSelections => currentSelections.filter(sel => {
      if (sel.salle !== mergedSlot.salle) return true;
      const isInside = (sel.startHour >= mergedSlot.startHour - 0.001) && (sel.endHour <= mergedSlot.endHour + 0.001);
      return !isInside;
    }));
  };
  ...
  const preMergeSelections = (rawSelections) => {
    const bySalle = {};
    ...
    for (const salle in bySalle) {
      ...
      while (i < slots.length) {
        ...
        while (i + 1 < slots.length && Math.abs(current.endHour - slots[i + 1].startHour) < 0.01) {
  ...
  const checkConflicts = (candidates, allExistingReservations) => {
    ...
    candidates.forEach(candidate => {
      ...
      const hasConflict = allExistingReservations.some(existing => {
        ...
        if (normalizeRoomName(existing.salle) !== normalizeRoomName(candidate.salle)) return false;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      mergedSelections.forEach(sel => {
        ...
        if (formData.recurrence && formData.recurrenceJusquau) {
          const dates = generateRecurrenceDates(currentDate, new Date(formData.recurrenceJusquau), formData.recurrenceType);
          dates.forEach(date => {
            const dateRecurStr = googleSheetsService.formatDate(date);
            allCandidates.push({ ...baseRes, dateDebut: dateRecurStr, dateFin: dateRecurStr });
          });
        }
      });
    ...
    } catch (e) { alert("Erreur : " + e.message); setIsSubmitting(false); }
  };
