...
function ReservationGrid({ selectedDate, editReservationId, onBack, onSuccess }) {
  // ... (Code logique inchangé, le fix doublon est déjà intégré) ...
  ...
  // FIX DOUBLONS : Vérification strictes
  ...
  const removeSelection = (id) => setSelections(selections.filter((_, i) => i !== id));
  ...
  const handleReservationMouseEnter = (res, e) => { const rect = e.target.getBoundingClientRect(); setPopupPosition({ x: rect.left + window.scrollX + (rect.width / 2), y: rect.top + window.scrollY }); setHoveredReservation(res); };
  ...
  return (
    ...
    <div className="reservation-grid-container">
      {/* HEADER AVEC STRUCTURE FLEX 3 PARTIES EQUILIBREES */}
      <div className="date-navigation-bar">
        ...
        {/* IMPORTANT : PARTIE DROITE POUR EQUILIBRE */}
        ...
        <div className="form-column">
          {selections.length > 0 ? (
            <div className="reservation-form">
                <h3>{isEditMode ? 'Modifier' : 'Nouvelle Réservation'}</h3>
